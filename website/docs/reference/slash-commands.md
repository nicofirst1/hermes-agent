...
| `/split [name] [--inplace]` | **CLI only.** Branch the session AND open the branch in a new herdr split pane (or a new terminal window when not running inside herdr) while the current window stays on the parent session — a parallel sandbox for side questions and risky tangents without losing your place. Inside [herdr](https://herdr.dev) the branch boots in a pane next to the current one; elsewhere the best available terminal window is opened with `hermes --resume <branch_id>`; with no window mechanism (SSH, headless) the resume command is printed instead — the current window is never switched. `--inplace` falls back to `/branch` behaviour (switch this window). Refused mid-turn like `/branch`. The branch is a snapshot fork: later messages in either session do not sync. |
...
| `/split [name] [--inplace]` | **CLI only.** Branch the session and open the branch in a new herdr split pane (or terminal window) while the current window stays on the parent. See the full description in the classic-CLI table above. |
